...
# 1. 清空 Redis 队列
    # 在终端运行：
    # cd d:\GameDownload\Enterprise-Automation-Agent
    # python tasks.py clear
# 2.运行监听esc，主动停止程序   cilent.py
# 3. 重新启动 Celery worker（新终端）
    # cd d:\GameDownload\Enterprise-Automation-Agent
    # python -m celery -A tasks.celery_app worker --loglevel=info -c 1 -P solo
# 4. 启动 FastAPI 服务（另一个新终端）
    # cd d:\GameDownload\Enterprise-Automation-Agent
    # uvicorn tasks:api_server --host 127.0.0.1 --port 8111
...
def data_cleaning_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
...
def llm_summary_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
