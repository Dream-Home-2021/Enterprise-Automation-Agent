"""WorkflowManager — 父图 + 分析子图 + 聊天子图，统一用 RedisSaver。

检查点策略：
  - 所有子图和父图统一使用同一个 checkpointer（外部注入的 AsyncRedisSaver）
  - 不再使用 MemorySaver，确保重启后状态可恢复，无孤儿 checkpoint
"""
...
class WorkflowManager:
    ...
    def _build_analysis_subgraph(self, checkpointer=None):
        ...
        # HumanChoice 使用 interrupt() 暂停，恢复后根据 state 路由
    ...
    def setup_workflow(self, checkpointer=None):
        print(f"[DEBUG] 传入的 checkpointer = {checkpointer}")  # 加这行
