"""
长期记忆 — PostgreSQL 读写封装。

提供高级 API 供 Graph 节点和 Service 层使用。

写入管线：
- save_conversation_memory() — 保存对话摘要（供后台批量提取调用）
- load_relevant_memories() — 向量 RAG 检索

读取管线（被 supervisor 和 chat_agent 使用）：
- load_user_memory() — 加载所有结构化长期记忆
- load_profile_inject() — 格式化为 system prompt 可注入文本
"""
